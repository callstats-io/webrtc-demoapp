...
#### Functions
  ...
  * ```toggleAVStates(isMuteOrPaused, isAudio)``` - Toggle audio,video states.Is muted or paused defines the
...
##### for cs.js
  ...
  * ```toggleAVStates({'userId', 'pc','type'})``` - Audio,video state was changed
